...
use chrono::{Local, DateTime, NaiveDate, NaiveDateTime, Timelike};
...
impl EventHandler for Handler {
    async fn message(&self, ctx: Context, msg: Message) {
        if msg.content == "!wp" {
            let strings = Ok::<String, anyhow::Error>("good morning".to_string());

            match strings {
                Ok(s) => {
                    let res = msg.channel_id.say(&ctx.http, follow_cutting_string(s)).await;
                    match res {
                        Ok(_) => (),
                        Err(e) => eprintln!("{:?}", e)
                    }
    ...
    async fn ready(&self, context: Context, ready: Ready) {
        ...
        tokio::spawn(async move {
            loop {
                ...
                if now.time().hour() > target_hour || (now.time().hour() == target_hour && now.time().minute() >= target_min) {
                ...
                let tomorrow_at_0005: NaiveDateTime = date.and_hms_opt(target_hour, target_min, 0).expect("failed to get time");
                // println!("{}", tomorrow_at_0005.format("%F %R"));

                let duration_to_wait = tomorrow_at_0005.signed_duration_since(now.naive_local());

                println!("next action at after {} min", duration_to_wait.num_minutes());

                tokio::time::sleep(duration_to_wait.to_std().unwrap()).await;

                let strings = Ok::<std::string::String, anyhow::Error>("hello".to_string());

                match strings {
                    Ok(s) => {
                        let res = channel_id.say(&context.http, follow_cutting_string(s)).await;
                        match res {
                            Ok(_) => (),
                            Err(e) => eprintln!("{:?}", e)
                        }
